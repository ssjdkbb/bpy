...
button_opened=False #串口按钮
cap_opened=True #摄像头打开
list_mn_black=[]# 黑子位置记录
moved_mn=False  # 移动状态判定
...
def robot_move_xy(x,y,v):
    ...
    for i in range(v):
        bpy.data.objects['Dof1'].rotation_euler[2] +=j1/v
        bpy.data.objects['Dof4'].rotation_euler[2] +=j2/v
...
class five2lua: 
    ...
    def  img2mask(self, img):
        ...
        upper_white = np.array([30, 200, 255])
...
def robot_process(x,y): #流程
    ...
    arms_up()
    robot_move_xy(x,y,40)
    arms_down()
    cancel_parent(copy,x,y)
    arms_up()
    robot_move_xy(xp,yp,40)
...
def mouse_control():
    ...
    if ma==0:
        ...
        thread1.start()
    if len(Vector)==3:
            if bpy.data.objects["Chessboard"].mode=='EDIT':
                count+=1
                print("count=",count)
                bpy.ops.object.editmode_toggle()
...
class eyes_control(bpy.types.Operator):
    ...
    def execute(self,context): #执行函数  
        global info
        if button_game:
            bpy.types.SpaceView3D.draw_handler_remove(info,'WINDOW') # 将模态处理函数在窗口管理器中移除
            print("Game Over!")
            if fivelua:
                fivelua.start=False
            Game_over()
        else:
            info = bpy.types.SpaceView3D.draw_handler_add(draw_callback_px, (None, None), 'WINDOW', 'POST_PIXEL') # 将文本绘制添加到窗口中
            Game_start()
            choose_object("Chessboard")
            bpy.ops.object.editmode_toggle()
            bpy.app.timers.register(mouse_control) 
            bpy.app.timers.register(move_control) 
            if fivelua:
                fivelua.start=True
        return {"FINISHED"}
class opened_control(bpy.types.Operator):
    ...
    def execute(self,context): #执行函数
        ...
        if button_opened:
            ...
            thread1 = threading.Thread(target=test)  # 运行识别函数
            thread1.start()
        return {"FINISHED"}
